...
function renderWorkSummary(facts: DayFactsLite): string {
  const commits = facts.git.repos.flatMap((repo) => repo.commits.map((commit) => ({ repo: repo.name, commit })))
  if (commits.length > 0) return `<div class="work-list">${commits.map(({ repo, commit }, index) => `<div class="work-row"><span class="work-index">${String(index + 1).padStart(2, '0')}</span><div><strong>${escapeHtml(workTitle(commit))}</strong><small>${escapeHtml(repo)} · ${commit.files} 文件 · +${commit.insertions}/-${commit.deletions}</small></div></div>`).join('')}</div>`
...
.work-list{display:grid;gap:0}.work-row{display:grid;grid-template-columns:34px minmax(0,1fr);gap:12px;align-items:start;padding:13px 0;border-top:1px solid var(--line)}.work-row:first-child{border-top:0;padding-top:0}.work-index{color:var(--blue);font:600 12px ui-monospace,SFMono-Regular,Menlo,monospace;padding-top:2px}.work-row strong{display:block;font-size:14px;line-height:1.5;overflow-wrap:anywhere}.work-row small{display:block;color:var(--muted);font-size:11px;margin-top:5px}
