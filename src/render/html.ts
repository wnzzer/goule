...
import { gouleIconInline } from './icon'
import {
  dayFactsMouse,
  escapeHtml,
  formatMinutes,
...
function renderWorkSummary(facts: DayFactsLite): string {
  const business = summarizeBusinessChanges(facts)
  if (business.length > 0) return `<p class="business-overview">${escapeHtml(businessOverview(business))}</p><div class="work-list">${business.map((item, index) => `<div class="work-row"><span class="work-index">${String(index + 1).padStart(2, '0')}</span><div><strong>${escapeHtml(item.title)}</strong><p class="business-requirement">需求：${escapeHtml(item.requirement)}</p><small>代码依据：${escapeHtml(item.source)} · ${escapeHtml(item.evidence)}</small></div></div>`).join('')}</div>`
  const titled = facts.sessions.filter((session) => session.title?.trim()).slice(0, 8)
  if (titled.length === 0) return '<div class="empty">今天没有可识别的交付项。</div>'
  return `<div class="work-list">${titled.map((session, index) => `<div class="work-row"><span class="work-index">${String(index + 1).padStart(2, '0')}</span><div><strong>进行中：${escapeHtml(session.title!.trim())}</strong><small>${escapeHtml(session.branch ?? '未识别分支')} · ${escapeHtml(formatMinutes(session.minutes))}</small></div></div>`).join('')}</div>`
}
...
<body><main>
<header class="hero"><div class="brand-row">${gouleIconInline(40)}<span class="eyebrow">GOULE · ENOUGH, CLOCK OUT</span></div><h1>${escapeHtml(facts.dayId)} 工作日报</h1><p class="subtitle">今天完成 ${facts.git.totals.commits} 项代码交付</p><div class="hero-meta"><span class="pill">${escapeHtml(facts.boundary.tz)}</span><span class="pill">逻辑日 ${facts.boundary.cutoffHour}:00 起</span><span class="pill">生成于 ${escapeHtml(new Date(facts.generatedAt).toLocaleString('zh-CN'))}</span></div></header>
<div class="metrics">
...
<section><div class="section-head"><div><span class="eyebrow">WHAT CHANGED</span><h2>今天做了什么</h2></div><p>先看交付结果</p></div>${renderWorkSummary(facts)}</section>
...
${unattributed > 0 ? `<section><div class="section-head"><div><span class="eyebrow">TRANSPARENCY</span><h2>未关联项</h2></div></div><p class="muted">${facts.unattributed.sessions.length} 个 session、${facts.unattributed.commits.length} 个 commit 未能关联到明确的工作流。</p></section>` : ''}
